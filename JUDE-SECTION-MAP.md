# Jude section map (movement level)

The planned studies for Jude, a single chapter divided by its flow from the call to contend for the faith, through examples of judgment on the ungodly, to a charge to persevere and a closing doxology. This fixes the `order` and `slug` for each study so they stay stable as we generate them one at a time. Status: done means the JSON exists in `src/content/studies/jude/`.
