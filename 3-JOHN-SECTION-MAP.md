# 3 John section map (movement level)

The planned studies for 3 John, a single short chapter divided by its flow as it commends Gaius, rebukes Diotrephes, and commends Demetrius. This fixes the `order` and `slug` for each study so they stay stable as we generate them one at a time. Status: done means the JSON exists in `src/content/studies/3-john/`.
