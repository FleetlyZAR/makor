# Nahum section map (movement level)

The planned studies for Nahum, divided by the book's seams: the opening hymn to the
LORD's avenging power, the word of judgment and comfort, and the two vivid oracles
describing Nineveh's fall. This fixes the `order` and `slug` for each study so they
stay stable as we generate them one at a time. Status: done means the JSON exists
in `src/content/studies/nahum/`.
