# Jonah section map (movement level)

The planned studies for Jonah, divided by the book's four narrative scenes: the
flight and the storm, the prayer from the fish, the preaching that turns Nineveh,
and the confrontation over God's compassion. This fixes the `order` and `slug` for
each study so they stay stable as we generate them one at a time. Status: done
means the JSON exists in `src/content/studies/jonah/`.
