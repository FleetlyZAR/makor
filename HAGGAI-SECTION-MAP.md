# Haggai section map (movement level)

The planned studies for Haggai, divided by the book's four dated oracles: the call
to rebuild the temple, the promise of its coming glory, the word on defilement and
blessing, and the final promise to Zerubbabel. This fixes the `order` and `slug`
for each study so they stay stable as we generate them one at a time. Status: done
means the JSON exists in `src/content/studies/haggai/`.
